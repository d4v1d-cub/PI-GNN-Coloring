...
q = int(sys.argv[1])   # This is the chromatic number to try for all the graphs
Nstr=sys.argv[2]

TrPath=sys.argv[3]
TePath=sys.argv[4]
test_every=int(sys.argv[5])
    ...
    for epoch in range(hypers['number_epochs']):
        ...
        for batchJ, batch in enumerate(train_dataloader[i]):
            ...
            try:
                ...
                InstaSaver(f'{save_path}/train',[epoch,detloss],f'q_{i}_{Nstr}_ndata_{len(data_train[i])}_TrData')
...
# | batch {teBatchJ} | #nodes: {test_batch.num_nodes} | GPU memory {torch.cuda.memory_allocated(device=TORCH_DEVICE)}')# | Soft Loss: {Tloss.item():.3f}
 ...
 | time:{round(time() - t_start, 4)}s')#| Soft Loss: {EpochCumLossPerBatchEval/len(test_dataloader[i]):.3f}
            InstaSaver(f'{save_path}/test',[epoch,EpochCumHardLossPerBatchEval/len(test_dataloader[i])],f'q_{i}_{Nstr}_ndata_{len(data_train[i])}_TstData')
