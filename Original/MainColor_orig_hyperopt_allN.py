...
dimmin = int(sys.argv[8])
dimmax = int(sys.argv[9])
doutmin = float(sys.argv[10])
doutmax = float(sys.argv[11])
lrmin = float(sys.argv[12])
lrmax = float(sys.argv[13])
...
space = (hp.quniform('dim', dimmin, dimmax, 1), 
        hp.uniform('dout', doutmin, doutmax), hp.uniform('lrate', lrmin, lrmax))

def train_all(dim, dout, lrate):
...
def objective(args):
    dim, dout, lrate = args
    dim = int(dim)
    print(f'\nWith parameters\n embdim={dim}\thiddim={dim}\tdout={dout}\tlrate={lrate}')
    loss = train_all(dim, dout, lrate)
    print(f'loss = {loss}\n')
    return loss
...
fout.write(f'{int(best["dim"])}\t{int(best["dim"])}\t{best["dout"]}\t{best["lrate"]}\n')
fout.close()